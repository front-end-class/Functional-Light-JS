...
这类技术通常称为尾调用优化(Tail Call optimization, TCO)，但重要的是要区分检测要在固定内存空间中运行的尾调用的能力和优化此方法的技术。从技术上讲，tail调用本身并不是大多数人认为的性能优化，因为它们实际上可能比正常调用运行得慢。TCO是关于优化尾调用以更有效地运行。

### 正确的尾调用 (PTC)

JavaScript从来没有要求(或禁止)尾调用，直到ES6。ES6要求对尾调用(称为适当的尾调用(PTC))的特定形式的尾调用进行识别，并保证PTC形式的代码在没有无限制堆栈内存增长的情况下运行。实际上，这意味着如果我们坚持PTC，就不会抛出RangeError。
